...
* identifier 1..* Identifier "Unique identifier for the procedure. For example: a unique identifier assigned to a procedure by a hospital's internal software system"
* recordedDate 1..1 dateTime "Date of the last modification/recording of the procedure"
* performedDate[x] 1..1 dateTime or Period "Date or period when the procedure was actually performed. For example: the date of an operation or the period during which a treatment was carried out"
* patient 1..1 Reference "The unique patient identification code. This includes the patient's Social Security Identification Number (NISS), generally corresponding to the national registry number. In some cases, another unique identification code may be authorized, such as the BIS number"
* recorder 1..1 Reference "Person, organization or device that recorded the procedure. For a healthcare provider, this is the Social Security Identification Number (NISS). For an organization, the Crossroads Bank for Social Security (BCSS) number is used. For a medical device, use the unique device identification number"
* performer 0..1 Reference "Person who performed the procedure. For a healthcare provider, this is the Social Security Identification Number (NISS). For an organization, the BCSS number is used. For a medical device, use the unique device identification number"
* basedOn 0..* Reference "Reference to the CarePlan and/or Referral Prescription. For example: a physiotherapy session performed on prescription from the general practitioner"
...
* focalDevice 0..* Reference "The device that was manipulated"
